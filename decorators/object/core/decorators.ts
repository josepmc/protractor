...
export function fillWith(targetProperty: string) {
    return (target: Object, appliedProperty: string | symbol) => {
        ...
        assert(!!targetGetFn, `Decorator Setup: Error! Can't set up a mirror property ${appliedProperty} for ${targetProperty}, the 'get' method doesn't exist`);
    ...
    return function (target: AbstractObject, property: string | symbol): void {
        ...
        Object.defineProperty(target, property, {
            ...
            set: function (newVal: T) {
                ...
                assert(setFn || newVal === undefined, `${property}: Can't set the value using this method, no setter found.`);
                if (newVal === undefined && _val !== undefined)
                    console.warn(`${property}: Attention! null =: Field is present, but no meaningful value is selected.
                ...
                if (_val instanceof AbstractObject) {
                    console.warn(`${property}: Attention! You are going to replace an AbstractObject object. This is highly discouraged. Was this call intended?.`);
                }
        ...
        target[generateName(FnNames.Get, property)] = async function (): Promise<T> {
            ...
            try {
                ...
                debugger;
                console.log(`Get - ${property}: An error ocurred`);
                if (err) throw err;
            }
            Reflect.defineMetadata(holder, _val, this, property);
            console.log(`Getting ${property} => ${_val}`);
            return _val;
        };
        ...
        target[generateName(FnNames.Set, property)] = async function (value: T, force: boolean = false): Promise<void> {
            ...
            if (fn) {
                value = await value;
                console.log(`Setting ${property} from ${_val} to ${value}`);
